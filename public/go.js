var projectso = async function() {
    ...
    const editorHtml = [
      '<div class="featureso-feature-overlay">',
        '<div class="featureso-wrapper">',
          ...
          '<p class="title">Feature.so is a no-code editor. Hover over an area on your website, and click to confirm where you would like the new feature to be added. The new feature will always be added after the element that you selected.</p>',
        '</div>',
      '</div>'
    ].join('');
  ...
  function sectionAdded(targetElement, featureId, accessToken){
    ...
    document.querySelector('.featureso-button__confirm').addEventListener("click", function(e) {
      ...
      .then(function (result) {
        console.log(result);
        if(result?.token_confirmed === true && result?.token_expired === false && result?.feature_added === true){
