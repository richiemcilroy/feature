var projectso = async function() {
  ...
  function sectionConfirm(parent, className){
    const featureOverlay = document.querySelector('.featureso-feature-overlay');
    let confirmDiv = document.createElement('div');
    confirmDiv.innerHTML = editorHtml;
    featureOverlay.appendChild(editorDiv);
  }
  ...
  function initialiseEditor(){
    const editorHtmlStyling = `
      ...
      .featureso-feature-element-highlight {
        border-bottom: 8px solid #2d8589 !important;
        cursor: pointer !important;
        transition: none !important;
      }
      .featureso-feature-element-highlighted {
    ...
    const editorHtml = [
      '<div class="featureso-feature-overlay">',
        ...
        '<p class="title">No-code content editor. Hover over your existing section, and click to confirm where you would like the new feature to be added. The new feature will always be appended <strong></strong> the section you selected.</p>',
      '</div>'
    ].join('');
    ...
    if (document.body.addEventListener) {
      ...
      document.body.addEventListener("click", function(e) {
        ...
        document.querySelectorAll('.featureso-feature-element-highlighted').forEach(function(highlight) {
          highlight.classList.remove("featureso-feature-element-highlighted");  
        });
        e.target.classList.add('featureso-feature-element-highlighted');

        sectionConfirm(target.offsetParent, target.className);
      });
    }
    ...
    function handler(event) {
      ...
      if (prev) {
        prev.className = prev.className.replace(/\bfeatureso-feature-element-highlight\b/, '');
        prev = undefined;
      }
      if (event.target) {
        prev = event.target;
        prev.className += " featureso-feature-element-highlight";
      }
    }
  }
